...
export function QuizModal({ taskId, isOpen, onClose, onSuccess, hasFullName }: QuizModalProps) {
  ...
  const handleSaveName = async () => {
    ...
    if (user) {
      await supabase.from('profiles').update({ nombre_completo: fullNameInput.trim() }).eq('id', user.id);
    }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-outline-variant/30 flex justify-between items-center bg-surface-container-lowest">
          ...
          <button onClick={onClose} className="p-2 rounded-full hover:bg-surface-container transition-colors text-outline">
            <X className="size-5" />
          </button>
        </div>
        ...
        <div className="p-6 md:p-8 min-h-[300px] flex flex-col">
          ...
          {step === 'name' && (
            <div className="flex-1 flex flex-col justify-center animate-in fade-in slide-in-from-bottom-4">
              ...
              <h2 className="text-xl font-bold text-center text-on-surface mb-2">Nombre para tu Certificado</h2>
              <p className="text-center text-sm text-on-surface-variant mb-6">
                Notamos que no has configurado tu nombre legal. Ingresa tu nombre completo real 
                tal y como quieres que aparezca impreso en tu certificado.
              </p>
              <input
                type="text"
                value={fullNameInput}
              ...
              >
                {isPending ? <Loader2 className="size-5 animate-spin mx-auto" /> : 'Guardar y Continuar'}
              </button>
            </div>
          )}
          ...
          {(step === 'loading' || step === 'evaluating') && (
            <div className="flex-1 flex flex-col items-center justify-center text-center animate-in fade-in">
              ...
              <h2 className="text-lg font-bold text-on-surface">
                {step === 'loading' ? 'Generando evaluación con IA...' : 'Evaluando tus respuestas...'}
              </h2>
                  ...
                  >
                    <div className="flex gap-3 items-start">
                      <div className={cn(
                        ...
                        selectedAnswers[currentQuestionIndex] === idx ? "border-primary bg-primary" : "border-outline"
                      )}>
                        {selectedAnswers[currentQuestionIndex] === idx && <div className="size-1.5 bg-white rounded-full" />}
                      </div>
                      <span>{opt}</span>
                    </div>
                  </button>

...
import { getGeminiClient, GEMINI_DEFAULT_MODEL, markActiveKeyExhaustedAndRotate, getGeminiKeyCount } from '@/lib/gemini/geminiClient';
import { z } from 'zod';
  ...
  try {
    ...
    while (attempt < maxAttempts) {
      try {
        ...
        console.error(`Error en intento ${attempt + 1}:`, err?.message || err);
        ...
        if (status === 429 || status === 503 || msg.includes('429') || msg.includes('503')) {
          ...
          if (attempt >= maxAttempts) {
            throw new Error('Todas las claves están ocupadas o el servicio está saturado. Intenta en un minuto.');
          }
        } else {
    ...
    if (!response) {
       return { success: false, error: 'No se obtuvo respuesta del modelo de IA tras varios intentos.' };
    }

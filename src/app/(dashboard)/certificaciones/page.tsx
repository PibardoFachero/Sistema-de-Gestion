...
import { Award, Calendar, Clock, CheckCircle, ArrowLeft, Download, Share2 } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col min-h-full pb-20 animate-in fade-in duration-500 max-w-5xl mx-auto">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {certs?.map((cert: any) => (
          <div key={cert.id} className="relative group bg-white border border-[#EAE3DC] rounded-[24px] overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] transition-all duration-300">
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-lg font-bold text-on-surface">{cert.horas_invertidas} horas</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-[11px] font-bold text-outline uppercase tracking-wider flex items-center gap-1">
                    <CheckCircle className="size-3" /> Tareas Aprobadas
                  </span>
                  <span className="text-lg font-bold text-on-surface">{cert.temas_aprobados} temas</span>
                </div>

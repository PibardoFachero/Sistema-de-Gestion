...
async function testAuthEndpointJson() {
  console.log(
    '1. Probando Endpoint de Autenticación en modo JSON (/api/calendar/auth?mode=json)...',
  );
  try {
...
async function testAuthEndpointRedirect() {
  ...
  try {
    ...
    assert(
      res.status === 307,
      `Código de respuesta esperado 307 Redirect, recibido: ${res.status}`,
    );
...
async function testEventsUnauthorized() {
  ...
  try {
    ...
    assert(
      res.status === 401,
      `Código de respuesta esperado 401 Unauthorized, recibido: ${res.status}`,
    );
    const data = await res.json();
...
function testZeroTrustEncryption() {
  ...
  try {
    ...
    const tokenCookie = Buffer.from(
      `${iv.toString('hex')}:${authTag}:${encrypted}`,
      'utf8',
    ).toString('base64url');

    assert(
      !tokenCookie.includes(sampleTokens.access_token),
      'El token no viaja en texto plano en la cookie',
    );
    ...
    assert(
      parsed.access_token === sampleTokens.access_token,
      'Integridad de access_token preservada tras descifrado',
    );
    assert(
      parsed.refresh_token === sampleTokens.refresh_token,
      'Integridad de refresh_token preservada tras descifrado',
    );
...
async function runAllTests() {
  ...
  console.log(
    `  Total: ${totalTests} | Aprobadas: \x1b[32m${passedTests}\x1b[0m | Fallidas: \x1b[31m${failedTests}\x1b[0m`,
  );
  console.log('========================================================\n');
